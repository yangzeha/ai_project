...
def evaluate(model, train_data, test_data, utils, device, model_type, top_k=20, user_history_state=None):
    model.eval()
    
    # [FIX] Use TRAIN data for graph propagation, NOT test data
    # This prevents data leakage. The model must predict test edges based on train graph.
    adj_matrix = utils.build_adj_matrix(train_data).to(device)
    
    # Mine bicliques for TRAIN data (for propagation context)
    # In a real scenario, we use the latest available bicliques from training
    H_v, H_u = None, None
    if model_type == "FullTSBCL":
        # We use a cached mining result or mine on the fly for the training snapshot
        # For simplicity here, we mine on train_data
        biclique_file = utils.run_msbe_mining(train_data, "eval_context", tau=2, epsilon=0.1)
        H_v, H_u = utils.parse_bicliques(biclique_file)
        H_v, H_u = H_v.to(device), H_u.to(device)
    
    test_users = list(set([x[0] for x in test_data]))
    # Filter users who are not in train_data (cold start users cannot be predicted by GCN easily)
    train_users = set([x[0] for x in train_data])
    test_users = [u for u in test_users if u in train_users]
    ...
    with torch.no_grad():
        ...
        for u in test_users:
            ...
            # Filter out items already seen in training (optional, but standard)
            # train_items = set([x[1] for x in train_data if x[0] == u])
    ...
    return hits / len(test_users) if len(test_users) > 0 else 0
...
def run_experiment(model_class, model_name, epochs=30):
    ...
    print(f"\n=== Running {model_name} (Fixed Evaluation) ===")
    ...
    # For evaluation context, we use the last training snapshot
    last_train_snapshot = train_snapshots[-1]
    ...
    for epoch in range(epochs):
        ...
        # Evaluate using the LAST training snapshot as context
        # We pass 'last_train_snapshot' as the graph structure
        recall = evaluate(model, last_train_snapshot, test_data, utils, device, model_name, user_history_state=user_history_state)
...
if __name__ == "__main__":
    ...
    plt.legend()
    plt.title('Recall Comparison (Fixed Evaluation)')
